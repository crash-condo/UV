...
int readings[25];  // Array to store the readings

//Function Preps
void blinkGREEN(int times);
void getCode();
void sendCode();
float getSoundLevelCB (int cbSize = 10);
...
float getSoundLevelCB (int cbSize = 10) {
  ...
  if (cbSize > 25) {
      Serial.println("Error: Maximum buffer size is 25.");
  ...
  for (int i = 0; i < cbSize; i++) {
    ...
    index = (index + 1) % cbSize;
  
    // Calculate the average
    average = total / (float)cbSize;
